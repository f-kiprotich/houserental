...
<script>
    ...
    $('#tenant_id').change(function() {
        ...
        $.ajax({
            ...
            success: function(resp) {
                try {
                    resp = JSON.parse(resp); // Safely parse JSON response
                    var details = $('#details_clone .d').clone();

                    // Populate tenant details
                    details.find('.tname').text(resp.name || 'N/A');
                    details.find('.price').text(resp.price || '0');
                    details.find('.outstanding').text(resp.outstanding || '0');
                    details.find('.total_paid').text(resp.paid || '0');
                    details.find('.rent_started').text(resp.rent_started || 'N/A');
                    details.find('.payable_months').text(resp.months || '0');

                    $('#details').html(details); // Update the details section
                } catch (error) {
            ...
            error: function(xhr, status, error) {
                ...
                alert("Failed to fetch tenant details. Please try again.");
            },
            complete: function() {
                end_load(); // End loading animation
            }
        });
    }); // Corrected closing of tenant_id change function
    ...
    $('#manage-payment').submit(function(e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            success: function(resp) {
                console.log(resp); // Debugging log
                if (resp == 1) {
