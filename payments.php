...
<?php
require 'mpesa_daraja.php';

if (isset($_POST['pay_rent'])) {
    $phoneNumber = $_POST['phone_number']; // Tenant's phone number
    $amount = $_POST['amount']; // Rent amount
    $accountReference = $_POST['tenant_id']; // Unique tenant identifier
    
    $mpesa = new MpesaDaraja();
    $response = $mpesa->stkPush($phoneNumber, $amount, $accountReference);

    if (isset($response->ResponseCode) && $response->ResponseCode == "0") {
        echo "Payment request sent. Please check your phone to complete the payment.";
    } else {
